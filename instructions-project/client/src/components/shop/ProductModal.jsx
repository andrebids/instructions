...
export default function ProductModal({ isOpen, onOpenChange, product, onOrder, enableQuantity = false }) {
  ...
  let baseNight = mapPath(activeProduct.images?.night || activeProduct.imagesNightUrl);
            ...
            <>
                    ...
                    {(usedPrice || usedStock) && (
                      <div className="mt-5 flex gap-2 border-b border-default-200 pb-2">
                        ...
                        <button
                          ...
                          className={`px-4 py-2 text-sm font-medium transition-colors ${productType === "used"
                            ? "text-primary border-b-2 border-primary"
                            : "text-default-500 hover:text-default-700"
                            }`}
                        >
              ...
              <ModalFooter>
                <div className="flex items-center gap-2">
                  ...
                  <Button color="primary" isDisabled={isOutOfStock} onPress={() => { onOrder?.(activeProduct, { mode, color }, enableQuantity ? qty : undefined); close(); }} className="bg-primary-500 text-white">Add</Button>
                  {isOutOfStock && (
