...
export function BioprintField({ bioprint, size = "base" }) {
  if (!bioprint) return null;
  ...
  const displayValue = Array.isArray(bioprint) ? bioprint.join(", ") : bioprint;

  return (
    <div className={`flex items-start ${gap}`}>
      <Icon icon="lucide:palette" className={`text-default-500 ${iconSize} mt-0.5`} />
      <div>
        <div className={`text-default-500 ${labelSize}`}>Bioprint</div>
