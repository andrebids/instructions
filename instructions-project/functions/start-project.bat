...
echo ======================================== > "%LOG_FILE%"
echo [%DATE% %TIME%] INICIANDO PROJETO >> "%LOG_FILE%" 2>nul
echo ======================================== >> "%LOG_FILE%" 2>nul
echo.
echo [DEBUG] Iniciando processo de inicializacao...
echo [%DATE% %TIME%] [DEBUG] Iniciando processo de inicializacao... >> "%LOG_FILE%" 2>nul
echo [DEBUG] Diretorio atual: %CD%
echo [%DATE% %TIME%] [DEBUG] Diretorio atual: %CD% >> "%LOG_FILE%" 2>nul
echo [DEBUG] Diretorio do script: %~dp0
echo [%DATE% %TIME%] [DEBUG] Diretorio do script: %~dp0 >> "%LOG_FILE%" 2>nul
echo.
...
echo [%DATE% %TIME%] [DEBUG] Parando processos anteriores... >> "%LOG_FILE%" 2>nul
call "%~dp0process-utils.bat" :stop_quick
set "STOP_RESULT=%errorlevel%"
echo [%DATE% %TIME%] [DEBUG] stop_quick retornou: %STOP_RESULT% >> "%LOG_FILE%" 2>nul
if %STOP_RESULT% neq 0 (
    echo [DEBUG] Aviso: stop_quick retornou erro, mas continuando...
    echo [%DATE% %TIME%] [DEBUG] Aviso: stop_quick retornou erro, mas continuando... >> "%LOG_FILE%" 2>nul
) else (
    echo [DEBUG] Processos anteriores parados com sucesso
    echo [%DATE% %TIME%] [DEBUG] Processos anteriores parados com sucesso >> "%LOG_FILE%" 2>nul
)
echo [DEBUG] Continuando apos stop_quick...
echo [%DATE% %TIME%] [DEBUG] Continuando apos stop_quick... >> "%LOG_FILE%" 2>nul
echo.

rem Verificar pré-requisitos (Node, npm)
echo [DEBUG] Verificando pre-requisitos...
echo [%DATE% %TIME%] [DEBUG] Verificando pre-requisitos... >> "%LOG_FILE%" 2>nul
call "%~dp0prereqs-utils.bat" :ensure_prereqs
set "PREREQS_RESULT=%errorlevel%"
echo [%DATE% %TIME%] [DEBUG] ensure_prereqs retornou: %PREREQS_RESULT% >> "%LOG_FILE%" 2>nul
if %PREREQS_RESULT% neq 0 (
    echo [DEBUG] ERRO: Pre-requisitos nao atendidos
    echo [%DATE% %TIME%] [DEBUG] ERRO: Pre-requisitos nao atendidos >> "%LOG_FILE%" 2>nul
    pause
    exit /b 1
)
echo [DEBUG] Pre-requisitos verificados com sucesso
echo [%DATE% %TIME%] [DEBUG] Pre-requisitos verificados com sucesso >> "%LOG_FILE%" 2>nul
echo.
...
echo.
echo [%DATE% %TIME%] [DEBUG] Chamando check_and_install_dependencies... >> "%LOG_FILE%" 2>nul
...
echo [%DATE% %TIME%] [DEBUG] check_and_install_dependencies retornou: %DEPS_RESULT% >> "%LOG_FILE%" 2>nul
if %DEPS_RESULT% neq 0 (
    ...
    echo [%DATE% %TIME%] [DEBUG] Pausando antes de voltar ao menu... >> "%LOG_FILE%" 2>nul
    pause
    exit /b 1
)
echo.
echo OK: Dependencias verificadas e instaladas!
echo [%DATE% %TIME%] Dependencias verificadas e instaladas! >> "%LOG_FILE%" 2>nul
echo.
echo [DEBUG] Dependencias OK, aguardando 2 segundos...
echo [%DATE% %TIME%] [DEBUG] Dependencias OK, aguardando 2 segundos... >> "%LOG_FILE%" 2>nul
timeout /t 2 /nobreak >nul
echo [DEBUG] Continuando apos dependencias...
echo [%DATE% %TIME%] [DEBUG] Continuando apos dependencias... >> "%LOG_FILE%" 2>nul
echo.
...
echo [%DATE% %TIME%] [DEBUG] BYPASS ATIVO - Pulando verificacoes de BD e migrations >> "%LOG_FILE%" 2>nul
...
echo [%DATE% %TIME%] [DEBUG] Iniciando secao do backend... >> "%LOG_FILE%" 2>nul
rem Verificar se backend já está a correr
echo [DEBUG] Verificando se backend ja esta a correr...
echo [%DATE% %TIME%] [DEBUG] Verificando se backend ja esta a correr... >> "%LOG_FILE%" 2>nul
echo [%DATE% %TIME%] [DEBUG] Executando curl para verificar backend... >> "%LOG_FILE%" 2>nul
curl -s -m 2 http://localhost:5000/health >nul 2>&1
set "CURL_RESULT=%errorlevel%"
echo [DEBUG] curl retornou: %CURL_RESULT%
echo [%DATE% %TIME%] [DEBUG] curl retornou: %CURL_RESULT% >> "%LOG_FILE%" 2>nul
if not errorlevel 1 (
    echo OK: Backend ja esta a correr na porta 5000
    echo [%DATE% %TIME%] [DEBUG] Backend ja esta a correr, pulando inicializacao... >> "%LOG_FILE%" 2>nul
...
echo [%DATE% %TIME%] [DEBUG] Backend nao esta a correr, continuando... >> "%LOG_FILE%" 2>nul

rem Verificar se há processo Node.js a usar a porta 5000
echo [DEBUG] Verificando se porta 5000 esta em uso...
echo [%DATE% %TIME%] [DEBUG] Verificando se porta 5000 esta em uso... >> "%LOG_FILE%" 2>nul
...
echo [%DATE% %TIME%] [DEBUG] Verificando porta 5000 de forma simplificada... >> "%LOG_FILE%" 2>nul
netstat -ano | findstr ":5000" | findstr "LISTENING" >nul
if errorlevel 1 (
    echo [DEBUG] Porta 5000 nao esta em uso - continuando...
    echo [%DATE% %TIME%] [DEBUG] Porta 5000 nao esta em uso - continuando... >> "%LOG_FILE%" 2>nul
    goto porta_livre
)
echo [DEBUG] Porta 5000 esta em uso - verificando processo...
echo [%DATE% %TIME%] [DEBUG] Porta 5000 esta em uso - verificando processo... >> "%LOG_FILE%" 2>nul
for /f "tokens=5" %%a in ('netstat -ano ^| findstr ":5000" ^| findstr "LISTENING"') do (
    echo [DEBUG] Verificando PID: %%a
    echo [%DATE% %TIME%] [DEBUG] Verificando PID: %%a >> "%LOG_FILE%" 2>nul
    tasklist /fi "PID eq %%a" 2>nul | findstr /i "node.exe" >nul
    if not errorlevel 1 (
        echo    Processo Node.js encontrado na porta 5000 (PID: %%a)
        echo [%DATE% %TIME%] [DEBUG] Processo Node.js encontrado, pulando inicializacao... >> "%LOG_FILE%" 2>nul
...
echo [%DATE% %TIME%] [DEBUG] Continuando para iniciar backend... >> "%LOG_FILE%" 2>nul
echo [DEBUG] Porta 5000 nao esta em uso, continuando...
echo [%DATE% %TIME%] [DEBUG] Porta 5000 nao esta em uso, continuando... >> "%LOG_FILE%" 2>nul
echo [DEBUG] Chegou ao ponto de iniciar o backend...
echo [%DATE% %TIME%] [DEBUG] Chegou ao ponto de iniciar o backend... >> "%LOG_FILE%" 2>nul

echo Iniciando servidor backend...
echo [%DATE% %TIME%] [DEBUG] Iniciando servidor backend... >> "%LOG_FILE%" 2>nul
echo [DEBUG] Configurando variaveis do backend...
rem Configurar NODE_OPTIONS para preferir IPv4 (resolve problema DNS Supabase)
set "NODE_OPTIONS=--dns-result-order=ipv4first"
echo [DEBUG] NODE_OPTIONS configurado: %NODE_OPTIONS%
echo [%DATE% %TIME%] [DEBUG] NODE_OPTIONS configurado: %NODE_OPTIONS% >> "%LOG_FILE%" 2>nul
echo [DEBUG] Usando comando start simples (versao funcional)...
echo [%DATE% %TIME%] [DEBUG] Executando comando start para backend... >> "%LOG_FILE%" 2>nul
...
echo [%DATE% %TIME%] [DEBUG] Criando arquivo batch temporario: %TEMP_BACKEND_BAT% >> "%LOG_FILE%" 2>nul
echo [%DATE% %TIME%] [DEBUG] Diretorio do backend: %BACKEND_DIR% >> "%LOG_FILE%" 2>nul
(
echo @echo off
echo title Backend Server
echo set NODE_OPTIONS=--dns-result-order=ipv4first
echo cd /d "%BACKEND_DIR%"
echo rem Usar npx para garantir que nodemon seja encontrado
echo rem npx resolve automaticamente o caminho do nodemon no node_modules/.bin
echo npx nodemon src/app.js
echo pause
) > "%TEMP_BACKEND_BAT%"
if not exist "%TEMP_BACKEND_BAT%" (
    echo [DEBUG] ERRO: Nao foi possivel criar arquivo batch temporario
    echo [%DATE% %TIME%] [DEBUG] ERRO: Nao foi possivel criar arquivo batch temporario >> "%LOG_FILE%" 2>nul
    exit /b 1
)
echo [DEBUG] Arquivo batch criado com sucesso
echo [%DATE% %TIME%] [DEBUG] Arquivo batch criado com sucesso >> "%LOG_FILE%" 2>nul
echo [DEBUG] Executando start com arquivo batch temporario...
echo [%DATE% %TIME%] [DEBUG] Executando start com arquivo batch temporario... >> "%LOG_FILE%" 2>nul
...
echo [%DATE% %TIME%] [DEBUG] DEPOIS de executar start - janela deve ter aberto >> "%LOG_FILE%" 2>nul
echo [DEBUG] Chegou apos verificacao do start...
echo [%DATE% %TIME%] [DEBUG] Chegou apos verificacao do start... >> "%LOG_FILE%" 2>nul
echo OK: Servidor backend iniciado em http://localhost:5000
echo.
echo    Aguardando servidor estar pronto (5 segundos)...
echo [%DATE% %TIME%] [DEBUG] Aguardando servidor estar pronto... >> "%LOG_FILE%" 2>nul
echo [DEBUG] Executando timeout...
echo [%DATE% %TIME%] [DEBUG] Executando timeout... >> "%LOG_FILE%" 2>nul
timeout /t 5 /nobreak >nul
set "TIMEOUT_RESULT=%errorlevel%"
echo [DEBUG] Timeout concluido, errorlevel: %TIMEOUT_RESULT%
echo [%DATE% %TIME%] [DEBUG] Timeout concluido, errorlevel: %TIMEOUT_RESULT% >> "%LOG_FILE%" 2>nul
echo [DEBUG] Aguardamento concluido, continuando...
echo [%DATE% %TIME%] [DEBUG] Aguardamento concluido, continuando... >> "%LOG_FILE%" 2>nul || echo.
...
echo [%DATE% %TIME%] [DEBUG] Iniciando secao do frontend... >> "%LOG_FILE%" 2>nul
...
echo [%DATE% %TIME%] [DEBUG] Verificando se porta 3003 esta em uso... >> "%LOG_FILE%" 2>nul
...
if not errorlevel 1 (
    ...
    echo [%DATE% %TIME%] [DEBUG] Porta 3003 esta em uso, assumindo que frontend esta rodando >> "%LOG_FILE%" 2>nul
    set "FRONTEND_ALREADY_RUNNING=1"
    goto frontend_already_running
) else (
    echo [DEBUG] Porta 3003 nao esta em uso, iniciando frontend...
    echo [%DATE% %TIME%] [DEBUG] Porta 3003 nao esta em uso, iniciando frontend... >> "%LOG_FILE%" 2>nul
)
    ...
    if not errorlevel 1 (
        ...
        for /f "tokens=5" %%a in ('netstat -ano ^| findstr ":%%P" ^| findstr "LISTENING"') do (
            ...
            if not errorlevel 1 (
                ...
                echo [%DATE% %TIME%] [DEBUG] Frontend detectado na porta %%P, pulando inicializacao >> "%LOG_FILE%" 2>nul
                set "FRONTEND_ALREADY_RUNNING=1"
                goto frontend_already_running
            )
        )
...
echo [%DATE% %TIME%] [DEBUG] Iniciando cliente frontend... >> "%LOG_FILE%" 2>nul
rem Criar arquivo batch temporario para evitar problemas com aspas e caminhos com espacos
...
echo [%DATE% %TIME%] [DEBUG] Criando arquivo batch temporario: %TEMP_FRONTEND_BAT% >> "%LOG_FILE%" 2>nul
echo [%DATE% %TIME%] [DEBUG] Diretorio do frontend: %FRONTEND_DIR% >> "%LOG_FILE%" 2>nul
rem Verificar se o diretorio existe
if not exist "%FRONTEND_DIR%" (
    echo [DEBUG] ERRO: Diretorio do frontend nao existe: %FRONTEND_DIR%
    echo [%DATE% %TIME%] [DEBUG] ERRO: Diretorio do frontend nao existe: %FRONTEND_DIR% >> "%LOG_FILE%" 2>nul
    goto frontend_error
)
(
echo @echo off
...
if not exist "%TEMP_FRONTEND_BAT%" (
    ...
    echo [%DATE% %TIME%] [DEBUG] ERRO: Nao foi possivel criar arquivo batch temporario do frontend >> "%LOG_FILE%" 2>nul
    goto frontend_error
)
echo [DEBUG] Arquivo batch criado com sucesso
echo [%DATE% %TIME%] [DEBUG] Arquivo batch criado com sucesso >> "%LOG_FILE%" 2>nul
...
if errorlevel 1 (
    ...
    echo [%DATE% %TIME%] [DEBUG] ERRO ao executar start para frontend, errorlevel: %errorlevel% >> "%LOG_FILE%" 2>nul
) else (
    echo [DEBUG] Comando start executado para frontend com sucesso
    echo [%DATE% %TIME%] [DEBUG] Comando start executado para frontend com sucesso >> "%LOG_FILE%" 2>nul
)
...
echo [%DATE% %TIME%] [DEBUG] Erro ao iniciar frontend, mas continuando... >> "%LOG_FILE%" 2>nul
...
echo [%DATE% %TIME%] [DEBUG] Chegou ao final da inicializacao do frontend >> "%LOG_FILE%" 2>nul
echo ========================================
echo    PROJETO INICIADO COM SUCESSO!
echo ========================================
echo.
echo [%DATE% %TIME%] PROJETO INICIADO COM SUCESSO! >> "%LOG_FILE%" 2>nul
...
echo [%DATE% %TIME%] [DEBUG] Detectando porta do frontend... >> "%LOG_FILE%" 2>nul
...
if not errorlevel 1 (
    ...
    echo [%DATE% %TIME%] [DEBUG] Frontend detectado na porta 3003 >> "%LOG_FILE%" 2>nul
...
for %%P in (5173 4173 3000 3001 3002 3005) do (
    if "!FRONTEND_FOUND!"=="0" (
        ...
        if not errorlevel 1 (
            ...
            echo [%DATE% %TIME%] [DEBUG] Frontend detectado na porta %%P >> "%LOG_FILE%" 2>nul
            goto frontend_port_detected
        )
    )
)
...
if "%FRONTEND_FOUND%"=="0" (
    ...
    echo [%DATE% %TIME%] [DEBUG] Frontend nao detectado automaticamente, usando porta padrao 3003 >> "%LOG_FILE%" 2>nul
    set "FRONTEND_PORT=3003"
)

:frontend_port_detected
echo [DEBUG] Chegou ao label frontend_port_detected
echo [%DATE% %TIME%] [DEBUG] Chegou ao label frontend_port_detected >> "%LOG_FILE%" 2>nul
set "FRONTEND_URL=http://localhost:%FRONTEND_PORT%"
echo [DEBUG] FRONTEND_URL definido como: %FRONTEND_URL%
echo [%DATE% %TIME%] [DEBUG] FRONTEND_URL definido como: %FRONTEND_URL% >> "%LOG_FILE%" 2>nul
echo [DEBUG] Verificando FRONTEND_ALREADY_RUNNING: [%FRONTEND_ALREADY_RUNNING%]
echo [%DATE% %TIME%] [DEBUG] Verificando FRONTEND_ALREADY_RUNNING: [%FRONTEND_ALREADY_RUNNING%] >> "%LOG_FILE%" 2>nul

rem Usar goto para evitar problemas com if/else em batch
echo [DEBUG] Antes do IF statement...
echo [%DATE% %TIME%] [DEBUG] Antes do IF statement... >> "%LOG_FILE%" 2>nul
if "%FRONTEND_ALREADY_RUNNING%"=="1" (
    echo [DEBUG] Dentro do IF - FRONTEND_ALREADY_RUNNING=1
    echo [%DATE% %TIME%] [DEBUG] Dentro do IF - FRONTEND_ALREADY_RUNNING=1 >> "%LOG_FILE%" 2>nul
    echo OK: Frontend ja estava a correr na porta %FRONTEND_PORT%
    echo [%DATE% %TIME%] [DEBUG] Frontend ja estava a correr na porta %FRONTEND_PORT% >> "%LOG_FILE%" 2>nul
    goto after_frontend_wait
)
echo [DEBUG] Apos o IF - FRONTEND_ALREADY_RUNNING nao e 1, continuando...
echo [%DATE% %TIME%] [DEBUG] Apos o IF - FRONTEND_ALREADY_RUNNING nao e 1, continuando... >> "%LOG_FILE%" 2>nul

rem Se chegou aqui, frontend foi iniciado agora
echo [DEBUG] Entrando no bloco de aguardar frontend...
echo [%DATE% %TIME%] [DEBUG] Entrando no bloco de aguardar frontend... >> "%LOG_FILE%" 2>nul
echo Frontend: %FRONTEND_URL%
echo [%DATE% %TIME%] [DEBUG] Frontend iniciado, aguardando estar pronto... >> "%LOG_FILE%" 2>nul
...
echo [%DATE% %TIME%] [DEBUG] Aguardamento concluido >> "%LOG_FILE%" 2>nul

:after_frontend_wait
echo.
echo [DEBUG] Preparando para abrir navegador...
echo [%DATE% %TIME%] [DEBUG] Preparando para abrir navegador... >> "%LOG_FILE%" 2>nul
echo [DEBUG] Abrindo navegador em %FRONTEND_URL%...
echo [%DATE% %TIME%] [DEBUG] Abrindo navegador em %FRONTEND_URL%... >> "%LOG_FILE%" 2>nul
start "" "%FRONTEND_URL%"
set "BROWSER_START_RESULT=%errorlevel%"
echo [DEBUG] Comando start retornou errorlevel: %BROWSER_START_RESULT%
echo [%DATE% %TIME%] [DEBUG] Comando start retornou errorlevel: %BROWSER_START_RESULT% >> "%LOG_FILE%" 2>nul
if %BROWSER_START_RESULT% neq 0 (
    echo [DEBUG] Erro ao abrir navegador, tentando metodo alternativo...
    echo [%DATE% %TIME%] [DEBUG] Erro ao abrir navegador, tentando metodo alternativo... >> "%LOG_FILE%" 2>nul
    start "" "%FRONTEND_URL%"
)
echo.
echo OK: Frontend aberto no browser na porta %FRONTEND_PORT%!
echo [%DATE% %TIME%] Frontend aberto no browser na porta %FRONTEND_PORT%! >> "%LOG_FILE%" 2>nul
echo.
echo OK: Projeto iniciado com sucesso!
echo [%DATE% %TIME%] Projeto iniciado com sucesso! >> "%LOG_FILE%" 2>nul
echo [DEBUG] Script chegou ao final com sucesso
echo [%DATE% %TIME%] [DEBUG] Script chegou ao final com sucesso >> "%LOG_FILE%" 2>nul
echo.
...
echo [%DATE% %TIME%] [DEBUG] Aguardando usuario pressionar tecla... >> "%LOG_FILE%" 2>nul
pause
echo [%DATE% %TIME%] [DEBUG] Usuario pressionou tecla, voltando ao menu... >> "%LOG_FILE%" 2>nul
exit /b 0
