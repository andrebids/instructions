...
# Verificar se git pull funcionou
$gitPullSuccess = $false
$serverUpdateOutputLower = $serverUpdateOutput.ToLower()

# Verificar se a saída indica sucesso (git pull funcionou)
if ($serverUpdateOutput -match '\[OK\] Código do servidor atualizado com sucesso|\[OK\] Código do servidor está atualizado|commit atual') {
    $gitPullSuccess = $true
    Write-Host "[OK] Código do servidor atualizado no servidor remoto via git pull!" -ForegroundColor Green
} elseif ($serverUpdateOutputLower -match 'diretório.*\.git.*não encontrado|\.git.*não encontrado|git pull falhou|git fetch falhou|continuando sem atualizar') {
    # Git pull falhou - não há repositório Git ou git pull falhou
    Write-Host "[AVISO] Git pull não funcionou - enviando código do backend via SCP..." -ForegroundColor Yellow
    $gitPullSuccess = $false
} elseif ($LASTEXITCODE -ne 0) {
    # Exit code não-zero indica falha
    Write-Host "[AVISO] Git pull falhou (exit code: $LASTEXITCODE) - enviando código do backend via SCP..." -ForegroundColor Yellow
    $gitPullSuccess = $false
} else {
    # Se não há indicação clara de sucesso ou falha, ser conservador
    # IMPORTANTE: Quando há incerteza, sempre assumir falha e enviar via SCP
    # Isso garante que o código seja atualizado mesmo se git pull falhou silenciosamente
    # A presença de palavras-chave relacionadas a git não garante sucesso (pode ser mensagem de erro)
    Write-Host "[AVISO] Status do git pull não foi determinado com certeza - enviando código do backend via SCP para garantir atualização..." -ForegroundColor Yellow
    $gitPullSuccess = $false
}

# Se git pull não funcionou, enviar código do backend via SCP
if (-not $gitPullSuccess) {
    Write-Host ""
    Write-Host "=== 4.5.1. Enviar código do backend via SCP ===" -ForegroundColor Cyan
    Write-Host "Enviando código do servidor (server/) para o servidor remoto..." -ForegroundColor Gray
    ...
    try {
        ...
        if (-not (Test-Path ".\server")) {
            Write-Host "[ERRO] Diretório server não encontrado: $PSScriptRoot\server" -ForegroundColor Red
            Write-Host "   Continuando sem atualizar código do backend..." -ForegroundColor Yellow
        } else {
            # Criar diretório temporário no servidor para o backend
            $serverTempPath = "/tmp/server-backend-$(Get-Date -Format 'yyyyMMdd-HHmmss')"
            Write-Host "Criando diretório temporário no servidor: $serverTempPath" -ForegroundColor Gray
            
            $createServerDirCmd = "mkdir -p $serverTempPath && chmod 755 $serverTempPath"
            $createServerDirOutput = Invoke-SshCommand -User $sshUser -SshHost $sshHost -Key $sshKey -BashCommand $createServerDirCmd
            
            if ($LASTEXITCODE -ne 0) {
                Write-Host "[ERRO] Não foi possível criar diretório temporário no servidor" -ForegroundColor Red
                Write-Host "   Continuando sem atualizar código do backend..." -ForegroundColor Yellow
            } else {
                # Enviar código do backend (excluindo node_modules, .git, dist, etc.)
                Write-Host "Enviando código do backend (isto pode demorar)..." -ForegroundColor Gray
                Write-Host "   Excluindo: node_modules, .git, dist, *.log, .env.local" -ForegroundColor Gray
                
                # Usar rsync se disponível, senão usar scp com exclusões
                # Primeiro tentar rsync (mais eficiente)
                $useRsync = $false
                try {
                    $rsyncCheck = Invoke-SshCommand -User $sshUser -SshHost $sshHost -Key $sshKey -BashCommand "which rsync 2>/dev/null"
                    if ($LASTEXITCODE -eq 0 -and $rsyncCheck -match 'rsync') {
                        $useRsync = $true
                    }
                } catch {
                    $useRsync = $false
                ...
                # Variável para rastrear sucesso do envio
                $backendUploadSuccess = $false
                
                if ($useRsync) {
                    # Usar rsync (mais eficiente, suporta exclusões nativas)
                    Write-Host "   Usando rsync para transferência eficiente..." -ForegroundColor Gray
                    $rsyncExcludes = @(
                    ...
                    $sshCmd = if ($sshKey) { "ssh -i $sshKey -o StrictHostKeyChecking=no" } else { "ssh -o StrictHostKeyChecking=no" }
                    
                    $rsyncArgs = @("-avz", "--delete") + $rsyncExcludes + @(
                        "-e", $sshCmd,
                        ".\server/",
                        "${sshUser}@${sshHost}:${serverTempPath}/"
                    )
                    
                    try {
                        & rsync $rsyncArgs 2>&1 | Out-Host
                        if ($LASTEXITCODE -eq 0) {
                            Write-Host "[OK] Código do backend enviado via rsync!" -ForegroundColor Green
                            $backendUploadSuccess = $true
                        } else {
                            throw "rsync falhou com código $LASTEXITCODE"
                        }
                    } catch {
                        Write-Host "[AVISO] rsync falhou, tentando com scp..." -ForegroundColor Yellow
                        $useRsync = $false
                        $backendUploadSuccess = $false
                    }
                }
                ...
                if (-not $useRsync) {
                    # Usar scp (fallback)
                    Write-Host "   Usando scp para transferência..." -ForegroundColor Gray
                    
                    # Criar arquivo temporário com lista de exclusões para tar
                    ...
                    # Usar tar para criar arquivo com exclusões e enviar via SSH
                    Write-Host "   Criando arquivo tar com exclusões..." -ForegroundColor Gray
                    $tarFile = [System.IO.Path]::GetTempFileName() + ".tar.gz"
                    
                    Set-Location ".\server"
                    try {
                        # Criar tar.gz excluindo arquivos desnecessários
                        $tarArgs = @("czf", $tarFile, "--exclude-from=$excludeFile", ".")
                        $null = & tar $tarArgs 2>&1
                        $tarExitCode = $LASTEXITCODE
                        
                        if ($tarExitCode -eq 0 -and (Test-Path $tarFile)) {
                            # Enviar arquivo tar.gz
                            Write-Host "   Enviando arquivo comprimido..." -ForegroundColor Gray
                            $scpOutput = Invoke-ScpCommand -Source $tarFile -Destination "$serverTempPath/backend.tar.gz" -User $sshUser -SshHost $sshHost -Key $sshKey
                            $scpExitCode = $LASTEXITCODE
                            
                            if ($scpExitCode -eq 0 -or $null -eq $scpExitCode) {
                                # Extrair no servidor
                                Write-Host "   Extraindo código no servidor..." -ForegroundColor Gray
                                $extractCmd = "cd $serverTempPath && tar xzf backend.tar.gz && rm -f backend.tar.gz && echo '[OK] Código extraído'"
                                $extractOutput = Invoke-SshCommand -User $sshUser -SshHost $sshHost -Key $sshKey -BashCommand $extractCmd
                                $extractExitCode = $LASTEXITCODE
                                
                                if ($extractExitCode -eq 0 -or $null -eq $extractExitCode) {
                                    Write-Host "[OK] Código do backend enviado via scp!" -ForegroundColor Green
                                    $backendUploadSuccess = $true
                                } else {
                                    throw "Falha ao extrair código no servidor (exit code: $extractExitCode)"
                                }
                            } else {
                                throw "Falha ao enviar arquivo para servidor (exit code: $scpExitCode)"
                            }
                            
                            # Limpar arquivo temporário local
                            Remove-Item -Path $tarFile -Force -ErrorAction SilentlyContinue
                        } else {
                            throw "Falha ao criar arquivo tar (exit code: $tarExitCode)"
                        }
                    } catch {
                        Write-Host "[ERRO] Erro ao enviar código do backend: $_" -ForegroundColor Red
                        Write-Host "   Continuando sem atualizar código do backend..." -ForegroundColor Yellow
                        $backendUploadSuccess = $false
                    } finally {
                        Set-Location $PSScriptRoot
                    ...
                    # Se usou rsync, assumir sucesso
                    $backendUploadSuccess = $true
                }
                
                # Mover código do diretório temporário para o local correto no servidor
                if ($backendUploadSuccess) {
                    Write-Host "Atualizando código do backend no servidor..." -ForegroundColor Gray
                    $moveServerCodeCmd = @"
cd $serverRootPath
if [ -d server ]; then
    # Fazer backup do servidor atual (apenas se não for muito grande)
    if [ -d server/node_modules ]; then
        echo '[INFO] Mantendo node_modules existente no servidor'
        # Mover node_modules temporariamente
        mv server/node_modules /tmp/server-node_modules-backup-`$(date +%s) 2>/dev/null || true
    fi
    
    # Remover código antigo (mas manter .env se existir)
    if [ -f server/.env ]; then
        echo '[INFO] Preservando arquivo .env'
        cp server/.env /tmp/server-env-backup 2>/dev/null || true
    fi
    ...
    # Mover novo código
    if [ -d $serverTempPath ] && [ "`$(ls -A $serverTempPath 2>/dev/null)" ]; then
        mv $serverTempPath/* server/ 2>/dev/null || true
        mv $serverTempPath/.* server/ 2>/dev/null || true
    ...
    echo '[ERRO] Diretório server não encontrado em '$serverRootPath
    exit 1
fi
"@
                    
                    $moveOutput = Invoke-SshCommand -User $sshUser -SshHost $sshHost -Key $sshKey -BashCommand $moveServerCodeCmd
                    Write-Host $moveOutput -ForegroundColor Gray
                    
                    if ($LASTEXITCODE -eq 0) {
                        Write-Host "[OK] Código do backend atualizado no servidor via SCP!" -ForegroundColor Green
                    } else {
                        Write-Host "[AVISO] Pode ter havido problemas ao atualizar código do backend" -ForegroundColor Yellow
                        Write-Host "   Verifique manualmente: ssh $sshUser@$sshHost 'ls -la $serverRootPath/server'" -ForegroundColor Yellow
                    }
                }
            }
        }
